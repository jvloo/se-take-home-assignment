...
test('BotIdle is a transition, not emitted per tryAssign for already-idle bots', () => {
  ...
  expect(idleAfterBots).toHaveLength(1);
  expect(idleAfterBots[0]).toMatchObject({ type: 'BotIdle', botId: 2 });
  ...
  expect(allIdle).toHaveLength(2);
  expect(allIdle[0]).toMatchObject({ type: 'BotIdle', botId: 2 });
  expect(allIdle[1]).toMatchObject({ type: 'BotIdle', botId: 1 });
});
